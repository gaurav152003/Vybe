...
export const getallloops = async (req, res) => {
    try {
        const loops = await Loop.find({}).populate("author", "name username profileImage").populate("comments.author","name username profileImage").sort({createdAt: -1})
...
export const savedloop = async (req, res) => {
    try {
        ...
        const alreadysaved = user.savedloop.some(id => id.toString() === loopid.toString())
        if (alreadysaved) {
            user.savedloop=user.savedloop.filter(id=>id.toString() !== loopid.toString())
        }
        else {
